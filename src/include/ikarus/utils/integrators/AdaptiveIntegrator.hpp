  ...
  public:
    ...
    ScalarType adaptlobstp(T &&f, const double a, const double b, ScalarType fa, ScalarType fb, ScalarType is) {
      ...
      if (is + (i1 - i2) == is or mll <= a or b <= mrr) {
        ...
        return adaptlobstp(f, a, mll, fa, fmll, is) + adaptlobstp(f, mll, ml, fmll, fml, is)
               + adaptlobstp(f, ml, m, fml, fm, is) + adaptlobstp(f, m, mr, fm, fmr, is)
               + adaptlobstp(f, mr, mrr, fmr, fmrr, is) + adaptlobstp(f, mrr, b, fmrr, fb, is);
      }
    }
    ...
    auto integrate(T &&f, const double a, const double b, const double tol_) {
      ...
      m = (a + b) / 2.;
      h = (b - a) / 2.;
      std::array<ScalarType, 13> y
          = {f(a),          f(m - x1 * h),   f(m - alpha * h), f(m - x2 * h),    f(m - beta * h), f(m - x3 * h), f(m),
      ...
      ScalarType is
          = h
            * (.0158271919734802 * (y[0] + y[12]) + .0942738402188500 * (y[1] + y[11])
               + .155071987336585 * (y[2] + y[10]) + .188821573960182 * (y[3] + y[9]) + .199773405226859 * (y[4] + y[8])
               + .224926465333340 * (y[5] + y[7]) + .242611071901408 * y[6]);
